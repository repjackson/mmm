if Meteor.isClient
    ...
    Template.classroom_edit_expenses.events
        'click .select_expense': ->
            Session.set 'selected_expense_id', @_id
    ...
    Template.classroom_edit_expenses.helpers
        expense_class: ->
            if Session.equals('selected_expense_id',@_id) then 'active' else ''
        selected_expense: ->
            Docs.findOne Session.get('selected_expense_id')
        debit_types: ->
            Docs.find
                model:'debit_type'
                classroom_id: Router.current().params.doc_id
